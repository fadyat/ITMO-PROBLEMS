...
while S and R: # while not the end of one of the [R, S]
    if r.a < s.a:
        next(R) # write in R
    elif r.a == s.a
        print(r + s) # concat r + s
...
Соединение таблиц - дорого

Нормализация - преобразование отношений к виду, отвечающему нормальной форме

Нормальная форма - характеристика отношения
- Вложены друг в друга

Избыточность (не путать с дублированием) - 

| Студент | | | | |
| - | - | - | - | - |
| ФИО | Группа | Образовательная программа | Факультет | Форма обучения |

Группа - не избыточное дублирование

Программа - избыточное дублирование (можно узнать из номера группы)

- Траты памяти

Для какого-то из студентов группые можно хранить ОП (не поможет с освобождением памяти)

Аномалии:
- аномалия-модификация - изменение значения одной записи повлечет за собой просмотр всей таблицы и изменение некоторых других записей
...
Комбинация [ФИО + Группа] - уникальна (первичный составной ключ)

В таблице сверху все плохо

- Функциональная зависимость между атрибутами - в отношении $R$ атрибут $Y$ функционально зависит от атрибута $X$ тогда и только тогда, когда каждому значению атрибута $X$ соответствует в точности одно значение атрибута $Y$
> $X \rightarrow Y$ 
...
- Полная функциональная зависимость - неключевой атрибут зависит от всего составного ключа

> {ФИО + Группа} $\rightarrow$ Форма обучения

- Транзитивная функциональная зависимость - существует функциональная зависимость из X в Z, если существует такое множетсво атрибутов Y такое, что есть ФЗ из X в Y и Y в Z

> $X \rightarrow Z, \exists Y: X \rightarrow Y, Y \rightarrow Z$

Нормальные формы:
- Первая нормальная форма - отношение находится в ПНФ, если всего его атрибуты являются простыми

|  | | | | Студент | | |
| - | - | - | - | - | - | - |
| Ф | И | О | Группа | Образовательная программа | Факультет | Форма обучения |

- Вторая нормальная форма - отношение находится во ВНФ, если оно находится в ПНФ и каждый неключевой атрибут функционально-полно зависит от первичного ключа
...
- Третья нормальная форма - отношение находится в ТНФ, если оно находится во ВНФ и все неключевые атрибуты взаимнонезависимы и полностью зависят от первичного ключа
- ТНФ - отношение находится в ТНФ, если оно находится во ВНФ и ни один неключевой атрибут не находится в транзитивной функциональной зависимости от первичного ключа
...
- Нормальная форма Бойса-Кодда - отношение находится в НФ Бойса-Кодда, если детерминанты(зависимые части) всех зависимостей являются потенциальными ключами.

| Проекты | | | | 
| - | - | - | - | 
| Номер студента | ФИО | Номер проекта | Роль | 

> Считаем ФИО неделимым, ФИО - уникально, не добваляем Группу  
> Потенциальный ключ: Номер студента + Номер проекта | ФИО + Номер проекта  
> Соответствует ТНФ  
> Не соответствует НФ Бойса-Кодда  

Приведенная к НФ Бойса-Кодда:
...
- Четвертая нормальная форма - отношение находится в ЧНФ, если оно находится в НФБК и не содержит многозначных зависимостей
...
Есть ПНФ и ШНФ - экзотичная организация данных

ТНФ - must have

Денормализация - искуственная композиция между отношениями для уменьшения операция соединения при запросах
